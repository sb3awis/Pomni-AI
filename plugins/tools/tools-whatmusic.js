...
const handler = async (m, { conn }) => {
  ...
  const traductor = JSON.parse(fs.readFileSync(`./src/ar.json`)).plugins.herramientas_whatmusic;
  ...
  if (/audio|video/.test(mime)) {
    ...
    // معالجة القيم التي قد تكون undefined
    const songTitle = title || traductor.texto2;
    const songSubtitle = subtitle || traductor.texto2;
    const songGenre = genres?.primary || traductor.texto2;
    
    const texto = `${traductor.texto3[0]}\n\n${traductor.texto3[1]} ${songTitle}\n${traductor.texto3[2]} ${songSubtitle}\n${traductor.texto3[4]} ${songGenre}`;
...
handler.command = ["tools-whatmusic"];
